...
El desarrollo del proyecto se organiza mediante Kanban, un método de gestión basado en un flujo continuo de tareas con límite de trabajo en curso, sin iteraciones de duración predeterminada. La elección responde a dos condiciones concretas del trabajo. En primer lugar, el equipo está conformado por una única persona, con lo cual las ceremonias propias de Scrum, orientadas a la sincronización entre varios integrantes, carecen de interlocutor. En segundo lugar, la cadencia del proyecto la imponen las cinco instancias de evaluación pautadas por la cátedra, separadas entre seis y ocho semanas y de alcance heterogéneo, lo que impide alinearlas con iteraciones de duración fija.
...
El clasificador opera sobre tres clases. La clase \textit{verdadero} agrupa las afirmaciones que se corresponden con la evidencia disponible; la clase \textit{falso}, aquellas que la evidencia contradice; y la clase \textit{sin verificar}, aquellas sobre las que no existe evidencia suficiente para pronunciarse. Esta tercera clase no representa un punto intermedio de una escala de veracidad sino la ausencia de respaldo, y se modela como clase propia y no como un umbral de confianza porque el contenido de circulación reciente, que es el objeto de interés del sistema, se ubica en esa situación con alta frecuencia.

Corresponde distinguir estas tres clases del veredicto que el usuario percibe, descrito en la Sección~\ref{sec:requerimientos}. El veredicto lo produce el módulo de orquestación combinando la salida del clasificador con el resultado del contraste externo y con las señales de trayectoria de la cuenta, así que su conjunto de estados no coincide con el del clasificador.
...
El segundo nivel emplea el \textit{Spanish Fake News Corpus} \parencite{PosadasEtAl2019}, con 971 muestras. Ese volumen no alcanza para entrenar un clasificador desde su inicialización, pero es adecuado como segundo ajuste sobre un modelo que ya resolvió la tarea en otra lengua: en esa instancia el modelo aprende de qué manera se expresa la desinformación en español, y no en qué consiste. La limitación del recurso está declarada en el Capítulo~\ref{ch:antecedentes} y condiciona el alcance de este nivel: sus fuentes provienen de México y España, con escasa representatividad del español rioplatense.

\subsection{Tercer nivel: corpus argentino}

El tercer nivel corresponde al corpus de publicaciones argentinas, y su método de construcción es una decisión de diseño, no una tarea de recolección independiente. El sistema persiste cada publicación que analiza junto con el resultado del análisis y la evidencia recuperada, según establece RF-16. Esa estructura coincide con la de un ejemplo de entrenamiento, de modo que el corpus se acumula como subproducto de la operación del sistema.

El corpus se organiza en dos subconjuntos de propósito diferenciado. El subconjunto de adaptación, previsto entre 2.000 y 5.000 publicaciones, se etiqueta a partir del veredicto producido por el sistema, con revisión humana de los casos en que la confianza del clasificador es baja, y se destina al tercer ajuste del modelo. El subconjunto de prueba, previsto entre 300 y 500 publicaciones, se anota manualmente por duplicado y se reserva como conjunto de evaluación en sentido estricto, sin participación alguna en el ajuste de parámetros. El protocolo de anotación y de medición del acuerdo entre anotadores se detalla en la Sección~\ref{sec:validacion}.
...
\begin{longtable}{@{}p{1.3cm} p{10.2cm} p{2.4cm}@{}}
  ...
  RF-11 & El usuario debe poder informar que un veredicto es incorrecto, y el sistema debe registrar esos informes para la revisión de errores y el reentrenamiento del modelo. & Importante \\
...
Los valores comprometidos son objetivos de diseño y no mediciones: su verificación experimental corresponde a una etapa posterior del proyecto. Se expresan mediante un número y no mediante un adjetivo para que resulten verificables.
...
\textbf{Descripción:} el analista de una organización cliente consulta qué temas con contenido marcado circulan en el período, cómo evolucionan y qué cuentas concentran mayor volumen, y exporta el recorte de su interés. Este caso de uso vincula el modelo de negocio descrito en el Capítulo~3 con el producto: es aquello que efectivamente se comercializa.
...
Los repositorios son el único punto de acceso a la base de datos. La concentración no responde a un criterio de purismo: el buscador vectorial y la memoria intermedia consultan la misma base que persiste los análisis, y unificar el acceso evita que la elección de la extensión vectorial se propague hacia los módulos.
...
Adoptar Python en su versión 3.14 obedece al mismo criterio de vigencia. El marco de trabajo adoptado admite versiones a partir de la 3.10, pero esa serie alcanza su fin de vida durante el período del proyecto, así que comprometerse con ella habría sido comprometerse con una dependencia que expira antes de la entrega final.
...
El codificador de \textit{embeddings} produce vectores de 768 dimensiones, valor verificado contra el archivo de configuración del modelo. Tres criterios sostienen la elección: no incorpora un sistema externo adicional, dado que la plataforma de inferencia ya forma parte del despliegue; pertenece a la misma familia arquitectónica que el clasificador, lo que permite un único procedimiento de segmentación y preprocesamiento; y ofrece un punto intermedio entre calidad de la representación y tamaño del índice para el volumen previsto. Ese número determina el tipo de las columnas de \textit{embeddings} del modelo de datos descrito en la Sección~\ref{sec:modelo-datos}, y modificarlo obliga a reconstruir el índice.
...
La métrica principal es el valor F1 macro. La elección responde a la distribución de las clases y no a una convención: los conjuntos disponibles están desbalanceados y la clase \textit{sin verificar} es la menos frecuente, con lo cual un clasificador que nunca la predijera podría alcanzar una exactitud global elevada y resultar inútil precisamente en el caso de mayor interés, que es el contenido de circulación reciente sobre el cual no existe verificación disponible. El F1 macro promedia el desempeño por clase sin ponderar por frecuencia, con lo que penaliza ese comportamiento.

Tres métricas complementan la evaluación. La precisión y la exhaustividad calculadas por clase permiten localizar el tipo de error: un falso positivo sobre contenido verdadero acarrea un costo reputacional y legal superior al de un falso negativo, según se desprende del análisis de la Sección~\ref{sec:legal}. El área bajo la curva característica operativa, calculada en la modalidad de una clase contra el resto, es independiente del umbral de decisión y habilita la comparación entre modelos sin fijar previamente un punto de corte. La matriz de confusión, por último, distingue entre la confusión de \textit{verdadero} con \textit{falso}, que es el error grave, y la confusión de cualquiera de ambas con \textit{sin verificar}, de consecuencias menores.
...
Al conjunto anterior se agrega un modelo de lenguaje grande operando sin ejemplos previos, incorporado a partir de la recomendación registrada en la Sección~\ref{sec:entrevistas}. Su función es establecer el costo de oportunidad de la decisión de arquitectura, no competir en desempeño: si un modelo de propósito general alcanzara un valor F1 comparable sin requerir ajuste fino ni corpus anotado, la justificación del enfoque adoptado debería revisarse. La comparación contempla, además del desempeño, la latencia y el costo por consulta, dado que ambos condicionan la viabilidad de un servicio gratuito según se desarrolla en la Sección~\ref{sec:tecnologias}.
...
El umbral de aceptación se fija en un valor de 0,60, correspondiente a la categoría de acuerdo sustancial en la escala propuesta por \textcite{LandisKoch1977}. Un valor inferior señalaría una deficiencia del esquema de etiquetas antes que del modelo, y obligaría a revisar la guía de anotación antes de continuar. Los desacuerdos se resuelven por discusión entre los anotadores y se documenta el criterio aplicado en cada caso, para que el conjunto resultante sea auditable.
