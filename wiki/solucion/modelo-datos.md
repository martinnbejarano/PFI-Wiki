...
`analisis` es el centro del esquema: participa de nueve de las veinte relaciones que dibuja el DER, más la relación N:M derivada con `documento`. Es la consecuencia directa de RF-16, que exige que cada análisis quede atado a lo que lo produjo —modelo y configuración de pesos— y de la doble propiedad que introduce la plataforma B2B.
...
Se descartó hashear el identificador desde la captura: rompía RF-07 y la deduplicación del caché —los dos se apoyan en el identificador nativo para saber si un tuit ya fue analizado— y volvía irreproducible el corpus entero para resolver un problema de unas pocas filas.
...
| `tipos_afirmacion` | `text[]` | NOT NULL | RF-04 y RF-05, ruteo por tipo |
...
| `score_similarity` | `numeric(4,3)` | NULL | RF-05 a RF-05, solo flujo profundo |
| `score_final` | `numeric(4,3)` | NULL | RF-06 |
| `veredicto` | `enum(contradicho, sospechosa, verificado, sin_contraste)` | NULL | RF-06 |
| `parcial` | `boolean` | NOT NULL, `false` | RF-08 |
| `modulos_faltantes` | `text[]` | NULL | RF-08 |
| `profundidad` | `enum(automatico, completo)` | NOT NULL | Decisión 1 de producto |
| `id_modelo_version` | `bigint` | FK, NOT NULL | RF-16 |
| `id_config_ensamblado` | `bigint` | FK, NOT NULL | RF-16 |
| `uuid_usuario` | `uuid` | FK → `usuario_extension`, NULL | RF-10 |
| `id_organizacion` | `bigint` | FK → `organizacion`, NULL | RF-13 |
| `fecha_analisis` | `timestamptz` | NOT NULL | |
| `vigente_hasta` | `timestamptz` | NULL | RF-07, política diferida |

Restricción de tabla: exactamente una de `uuid_usuario` y `id_organizacion` está poblada.

**Una sola fila que se completa, no dos filas.** Una fila por tuit y versión de modelo. Nace en CU-01 con `score_nlp` y se completa con una actualización cuando corre el flujo profundo de CU-02; `profundidad` distingue un estado del otro. Dos filas habrían obligado a decidir cuál de las dos es *el* veredicto del tuit en el histórico de RF-10 y en la agregación por tema de RF-14, y esa pregunta no tiene buena respuesta.

**`veredicto` tiene cuatro valores y no tres.** Los tres niveles de RF-06 más `sin_contraste`, que es el estado que vuelve cumplible a RNF-06: cuando ningún módulo externo encontró fuente, el indicador se emite igual pero como ausencia declarada y sin porcentaje, en lugar de vestirse de veredicto.

**`parcial` y `modulos_faltantes` son ortogonales al veredicto, no parte de él.** Son cosas distintas: `sin_contraste` es que el módulo corrió y no encontró fuente; `parcial` es que el módulo no pudo correr. Pueden darse a la vez, así que necesitan campos separados. Meterlas en el mismo enumerado habría hecho que RF-08 dejara de significar lo que dice.
...
| `id_analisis` | `bigint` | FK, NOT NULL | RF-06 |
| `texto` | `text` | NOT NULL | RF-06 |
| `orden` | `int` | NOT NULL | RF-06, orden de presentación |
| `id_evidencia` | `bigint` | FK → `evidencia`, NULL | RF-06 |

RF-06 exige que cada razón derivada de evidencia externa lleve el enlace a la fuente que la respalda. La clave foránea nulable es exactamente esa condición: hay razones que salen del análisis lingüístico o de las señales de la cuenta y no derivan de ningún documento, y esas quedan sin enlace de forma legítima.
...
| `vector` | `vector(768)` | NULL | RF-05, índice HNSW |
...
| `url` | `text` | UNIQUE, NOT NULL | RF-09 |
| `titulo` | `text` | NULL | RF-09 |
| `texto` | `text` | NULL | RF-05 a RF-05 |
| `tipo_fuente` | `enum(oficial, medio, verificador)` | NOT NULL | RF-09, orden jerárquico |
| `id_fuente_oficial` | `bigint` | FK, NULL | RF-05 |
...
| `similitud` | `numeric(4,3)` | NULL | RF-05 |
| `extracto` | `text` | NULL | RF-09 |
...
3. **RF-09 se resuelve con un `ORDER BY`** sobre `tipo_fuente`, en lugar de una unión de tres tablas.
...
**Dos columnas y nada más.** Sin preferencias, sin fecha de última actividad y sin contador de consultas: todo eso engordaría el perfil justo en la tabla que hay que defender. La configuración de la extensión —umbrales y desactivación por sitio— no sale del almacenamiento local del navegador, así que no tiene tabla.
...
| `id_analisis` | `bigint` | FK, NOT NULL | RF-11 |
| `uuid` | `uuid` | FK → `usuario_extension`, NOT NULL | RF-11 |
| `tipo` | `enum(falso_positivo, falso_negativo)` | NOT NULL | RF-11 |
| `motivo` | `text` | NULL | RF-11 |
| `id_modelo_version` | `bigint` | FK, NOT NULL | RF-11 |
...
| Atributo | Tipo | Restricción | Origen |
|---|---|---|---|
| `id_modelo_version` | `bigserial` | PK | RF-16 |
| `nombre_checkpoint` | `text` | NOT NULL | RF-16 |
| `fecha_entrenamiento` | `date` | NOT NULL | RF-16 |
| `f1_macro` | `numeric(4,3)` | NULL | RNF-05 |
| `dataset` | `text` | NULL | RF-16 |
...
| Atributo | Tipo | Restricción | Origen |
|---|---|---|---|
| `id_config` | `bigserial` | PK | RF-16 |
| `peso_nlp`, `peso_source`, `peso_similarity` | `numeric(3,2)` | NOT NULL | RF-16 |
| `umbral_sospechoso` | `numeric(3,2)` | NOT NULL | RF-06 |
| `umbral_contradicho` | `numeric(3,2)` | NOT NULL | RF-06 |
| `vigente_desde` | `timestamptz` | NOT NULL | RNF-16 |

**Por qué los pesos y los umbrales tienen tabla propia.** Los pesos 0,4 / 0,2 / 0,4 y los umbrales 0,40 y 0,75 no tenían dónde vivir, y RF-16 exige asociar cada análisis a la versión del modelo **y** a la configuración de pesos. La tabla resuelve dos cosas de una vez: RF-16 queda cubierto entero —la versión de modelo cubría solo la mitad— y RNF-16 pasa a ser implementable, porque mover un umbral es insertar una fila y no volver a desplegar el servicio.
...
| Atributo | Tipo | Restricción | Origen |
|---|---|---|---|
| `id_organizacion` | `bigserial` | PK | RF-13 |
| `nombre` | `text` | NOT NULL | RF-13 |
| `plan` | `enum(free, pro, enterprise)` | NOT NULL | RF-13 |
| `cuota_mensual` | `int` | NOT NULL | RF-13 |
| `id_externo_idp` | `text` | NULL | RF-13 |
...
| Atributo | Tipo | Restricción | Origen |
|---|---|---|---|
| `id_usuario` | `bigserial` | PK | RF-13 |
| `id_organizacion` | `bigint` | FK, NOT NULL | RF-13 |
| `email` | `text` | NOT NULL | RF-13 |
| `subject_idp` | `text` | UNIQUE, NOT NULL | RF-13 |
| `rol` | `text` | NOT NULL | RF-13 |
| `fecha_alta` | `timestamptz` | NOT NULL | |
...
| Atributo | Tipo | Restricción | Origen |
|---|---|---|---|
| `id_api_key` | `bigserial` | PK | RF-13 |
| `id_organizacion` | `bigint` | FK, NOT NULL | RF-13 |
| `hash_clave` | `text` | NOT NULL | RNF-12 |
| `prefijo` | `text` | NOT NULL | RF-13, identificación sin revelar la clave |
| `fecha_emision` | `timestamptz` | NOT NULL | RF-13 |
| `fecha_revocacion` | `timestamptz` | NULL | RF-13 |
| `activa` | `boolean` | NOT NULL | RF-13 |
...
| Atributo | Tipo | Restricción | Origen |
|---|---|---|---|
| `id_consumo` | `bigserial` | PK | RF-13 |
| `id_api_key` | `bigint` | FK, NOT NULL | RF-13 |
| `id_analisis` | `bigint` | FK, NULL | RF-13 |
| `fecha` | `timestamptz` | NOT NULL | RF-13 |
| `endpoint` | `text` | NOT NULL | RF-13 |
| `resultado` | `enum(ok, cuota_agotada, clave_invalida)` | NOT NULL | RF-13 |
...
| `modelo_version` — `analisis` | 1 : N | RF-16 |
| `configuracion_ensamblado` — `analisis` | 1 : N | RF-16, RNF-16 |
...
| `modelo_version` — `reporte` | 1 : N | RF-11, paso 3 de CU-04 |
...
|---|---|---|
| `tuit` | RF-01, RF-07, RF-16 | CU-01, CU-02 |
| `cuenta` | RF-01, RF-03, RF-16, RNF-10 | CU-01, CU-02, CU-07 |
| `fuente_oficial` | RF-05 | CU-02 |
| `medio_confiable` | RF-05, RF-09 | CU-02, CU-03 |
| `analisis` | RF-02, RF-06, RF-07, RF-08, RF-16 | CU-01, CU-02, CU-06 |
| `razon` | RF-06 | CU-02, CU-03 |
| `claim` | RF-04, RF-05 | CU-02 |
| `documento` | RF-05, RF-09, RF-16 | CU-02, CU-03 |
| `evidencia` | RF-05, RF-06, RF-09 | CU-02, CU-03 |
| `usuario_extension` | RF-10, RF-11 | CU-04, CU-05 |
| `reporte` | RF-11 | CU-04 |
| `modelo_version` | RF-11, RF-16 | CU-01, CU-02, CU-04 |
| `configuracion_ensamblado` | RF-16, RNF-16 | CU-01, CU-02 |
| `organizacion` | RF-13 | CU-06, CU-07 |
| `usuario_b2b` | RF-13 | CU-07 |
| `api_key` | RF-13, RNF-12 | CU-06 |
| `consumo_api` | RF-13 | CU-06 |
...
**No hay entidad `tema`.** RF-14 pide «temas con mayor circulación», que no se deriva de `claim.tipo` —que tiene cinco valores fijos y es un ruteador de fuentes, no una taxonomía temática—. Se resuelve por agregación al vuelo sobre los *claims* del período. Queda escrito para que nadie invente esa tabla más adelante.

**No hay tabla de configuración del ciudadano ni de sesión.** Los umbrales del indicador y la desactivación por sitio viven en el almacenamiento local del navegador; el ciudadano no se autentica, así que no hay sesión que persistir.
...
**No hay entidad para verificadores profesionales.** El corpus de verificaciones previas de RF-05 vive en `documento` con `tipo_fuente = verificador`. La decisión de no acceder al sitio de Chequeado está argumentada en [[wiki/proyecto/restricciones-legales-eticas]].
